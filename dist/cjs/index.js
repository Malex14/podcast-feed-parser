...
const CLEAN = {
    ...
    duration: function (arr) {
        let times = arr[0].split(':'), sum = 0, mul = 1;
    ...
    episodeType: function (string) {
        if (string.length > 0) {
            switch (string.charAt(0).toUpperCase()) {
                case 'F':
                    return types_1.EpisodeType.FULL;
                case 'T':
                    return types_1.EpisodeType.TRAILER;
                case 'B':
                    return types_1.EpisodeType.BONUS;
            }
        }
        return undefined;
    },
    ...
    if (Array.isArray(val)) {
        ...
        for (const item of val) {
            if (typeof item === 'string') {
                ...
                items.push(Object.assign({ value: item._, attrs: item['$'] ? item['$'] : {} }, (Object.keys(finalTags).length > 0 ? { nestedTags: finalTags } : {})));
            }
        }
        return items;
    }
